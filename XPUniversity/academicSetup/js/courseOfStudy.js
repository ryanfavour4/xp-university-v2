...
const renderTable = (data) => {
  let coursesOfStudyDT = data ? data : coursesOfStudy;

  const tableBody = document.getElementById('table-body');
  tableBody.innerHTML = '';
  coursesOfStudyDT.forEach((course, id) => {
    ...
    row.innerHTML = `
        ...
        <td>
          ...
          <button
            ...
            class="btn btn-primary"
            onclick="populateEditCOurseOfStudyPopup(${id})"
          >Edit</button>
...
/**
 * Populates the table with coursesOfStudy
...
const addCourse = async (e) => {
  ...
  console.log(data);
  try {
    await axios.post('http://localhost:8097/api/v1/coursesOfStudy/add', data)
    addCourseForm.reset();
...
const populateEditCOurseOfStudyPopup = (index) => {
  const editForm = document.getElementById('editForm');
  const course = { ...coursesOfStudy[index] };
...
const deleteCourse = (index) => {
  ...
  const course = coursesOfStudy[index];
  try {
    axios.delete(`http://localhost:8097/api/v1/coursesOfStudy/${course.CourseId}`)
    const newDiv = document.createElement('div');
    newDiv.className = 'alert alert-success';
    newDiv.innerHTML = 'Course deleted successfully';
    container.prepend(newDiv);
    coursesOfStudy.splice(index, 1);
    renderTable();
    setTimeout(() => {
      newDiv.remove();
    }, 5000);
...
const updateCourse = async (index) => {
  ...
  data.CourseLevel = Number(data.CourseLevel);
  data.CourseSemester = Number(data.CourseSemester);
  data.CourseId = coursesOfStudy[index].CourseId;

  try {
    await axios.put(`http://localhost:8097/api/v1/coursesOfStudy`, data);
    coursesOfStudy[index] = data;
    renderTable();
    editForm.reset();
...
const search = async (e) => {
  ...
  try {
    const res = await axios.post('http://localhost:8097/api/v1/coursesOfStudy', data);
